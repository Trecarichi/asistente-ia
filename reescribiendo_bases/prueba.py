...
ic(CSV_PATH)
RAM = False  # Cambiar a True para usar base de datos en RAM
...
def process_prompt(prompt):
    ...
    ## Agregar lógica de busqueda por similitud en frases (0.40)
    ...
    if any(phrase in user_question.lower() for phrase in trigger_phrases_documentos) and municipio_data:
        ...
        respuesta_final = (
            f"La dirección para tramitar en el municipio de {municipio_data.get('MUNICIPIO', 'el municipio')} es: **{municipio_data.get('DIRECCION', 'Dirección no disponible')}**. El horario de atención es: **{municipio_data.get('HORARIO', 'Horario no disponible')}**.\n\n"
            "Puedes acercarte con la documentación necesaria. Si tienes más dudas, puedes contactar a la Escribanía General a través de los siguientes medios:\n\n"
            "* **Página web:** 🔗 Web oficial\n"
            "* **Correo electrónico:** 📧 escgral@egg.gba.gov.ar\n\n"
...
if __name__ == '__main__':
    ...
    if not db_conn:
        ...
        print("Hola, soy un asistente virtual del Ministerio de Justicia y Derechos Humanos de la Provincia de Buenos Aires. Puedo orientarte sobre el programa \"Mi Escritura, Mi Casa\". ¿Qué necesitas saber? Recuerda que también puedes consultar en: 🔗 Web oficial 📧 Correo: escgral@egg.gba.gov.ar.")
